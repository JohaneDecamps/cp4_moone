...
export default function SignupPage() {
    ...
    const onSubmit = async (data) => {
        try {
            await axios.post("http://localhost:3310/api/users/register", data);
        } catch (e) {
            console.error(e)
        }
    };
    ...
    return (
        ...
        {...register("password", {
            ...
            pattern:{
                value:  /^(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[^\w\d\s:])([^\s]){8,64}$/,
                message: "Le mot de passe doit contenir au minimum 8 caractères dont une majuscule, un nombre et un caractère spécial"
            }
        })}/> 
